...
class DashboardCreator:
    ...
    def prepare_all_comparison_data(self):
        ...
        comparison_data = {category: {'labels': [], 'data': []} for category in categories}
        
        versions = sorted(self.aggregated_data.keys())
        for version in versions:
            for category in categories:
                load_time = next((lt["Load Time (Seconds)"]
                                for lt in self.aggregated_data[version]["load_times"]
                                if lt["Table Name"] == category), 0)
                comparison_data[category]['labels'].append(version)
                comparison_data[category]['data'].append(load_time)
